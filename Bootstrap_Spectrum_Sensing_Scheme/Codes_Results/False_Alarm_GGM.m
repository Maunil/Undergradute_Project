...
%alpha = 0.1;% false alarm probability
%---------------------------------------------------------------
%
%---------------------------------------------------------------
...
%-------------------------------------------------------------------------
            ...
            %--------------------------------------------------------------
                ...
                mu = zeros(1,M); %Mean
                beta = zeros(1,M); %Beta
                rho = zeros(1,M); %If rho = 1 than it's laplacian distribution
                                  %If rho = 2 than it's gaussian distribution
                ...
                for k = 1:length(mu)
                    n(k,:) = complex(gg6(1,L,mu(k),beta(k),rho(k)),gg6(1,L,mu(k),beta(k),rho(k))); %Generate Generalized Gaussian Model
                end
                
            %Detection Process 
            %--------------------------------------------------------------
            for a = 1:9 % This loop Determines the Different S_N Ratio 
                        ...
                        for i=1:B  
                                ...
                                for sample = 1:11
                                    ...
                                    gamma(i) = New_Test_statistics(1,i) - Main_Test_statistics; % Threshold value
                                    
                                    if Main_Test_statistics>=gamma(i) 
                                       h1(a)=h1(a)+1;  
                                    else 
                                       h2(a)=h2(a)+1;
                                    end    
...
%disp(S_N);
%disp(h1);
plot(S_N,(h1/(300*1000)))% Detection Probablity 
